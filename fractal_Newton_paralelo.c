...
#define WIDTH 800 //Pixeles de ancho de la imagen
#define HEIGHT 800 //Pixeles de alto de la imagen
#define LIMIT 1.5 //Limite para el plano complejo
#define MAX_ITER 35 //Maximo de iteraciones del metodo de Newton
...
int main(int argc, char *argv[]) {
    ...
    //Reconstruimos las raices en los procesos que no son el 0
    if (rank != 0) {
        for (int i = 0; i < store.count; i++) {
    ...
    int total_pixeles = WIDTH * HEIGHT; //Cantidada total de pixeles a procesar
    ...
    int *convergencia_raices_locales = (int *)malloc(cant_pixeles_locales * sizeof(int)); //alamacena la raiz a la que converge cada pixel
    int *cant_iteraciones_locales = (int *)malloc(cant_pixeles_locales * sizeof(int)); //almacena la cantidad de iteraciones para cada pixel
    ...
    computation_end_time = MPI_Wtime();  // Tiempo después del cálculo
    

    // ESCRITURA PARALELA DEL ARCHIVO (cada proceso escribe su parte)    
    // Paso 1: Preparar los datos a escribir en un buffer local
...
int newton_raphson(double complex z, const double complex roots[], int roots_count, int *iterations) {
    for (int k = 0; k < MAX_ITER; k++) {
        ...
        *iterations += 1;
        ...
        // Verificar convergencia contra todas las raíces encontradas
